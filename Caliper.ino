...
/* LIBRARY  ------------------------------------------------------------------*/
#include "rm67162.h"
#include <TFT_eSPI.h>  //https://github.com/Bodmer/TFT_eSPIz
...
#define DATAPIN    13  // data pin digital caliper
...
String callURL(const String &Server = "api.geqo.it", const String &url = "/index.php?api=mytesttx", const String &body = "") {
  ...
  String encodedString = base64::encode(body);
    ...
    if (WifiUsable && NotTransmited) {
      ...
      String urlComplete = url + "&chn=WIFI" + "&sensors=" + encodedString;
      Serial.println("callURL:");
      Serial.println((String) "GET " + urlComplete + " HTTP/1.1");
      Serial.println((String) "Host: " + host);
    ...
    if (GPRSUsable && NotTransmited) {
      ...
      String urlComplete = url + "&chn=GPRS" + "&sensors=" + encodedString;
      Serial.println("callURL:");
      Serial.println((String) "GET " + urlComplete + " HTTP/1.1");
      Serial.println((String) "Host: " + host);
    ...
    if (LORAUsable && NotTransmited) {
      ...
      String urlComplete = url + "&chn=LORA" + "&sensors=" + encodedString;
      Serial.println("callURL:");
      Serial.println((String) "GET " + urlComplete + " HTTP/1.1");
      Serial.println((String) "Host: " + host);
...
void setup() {
  ...
  String responseImg = callURL("http://api.geqo.it/ingr.jpg");
  byte* imageData = (byte*)malloc(responseImg.length());
  if (!imageData) {
    Serial.println("Failed to allocate memory for image data");
  }
  for (unsigned int i = 0; i < responseImg.length(); i++) {
    imageData[i] = (byte)responseImg[i];
    Serial.print((byte)responseImg[i]);
  }
  */
  /*
